...
interface Props {
  ...
  onClose: () => void;
  /** 删完跳转目标（删的是当前正在看的项目时传 "/projects"）；不传则 router.refresh */
  redirectTo?: string;
  /** 删完额外回调（如 switcher mutate） */
  onDeleted?: () => void;
}

/** S17：删除项目确认弹窗（软删，可由所有者从归档恢复）。 */
export function DeleteProjectModal({ open, target, onClose, redirectTo, onDeleted }: Props) {
  ...
  const onConfirm = async () => {
    ...
    setDeleting(true);
    try {
      await projectsApi.archive(target.id);
      ...
      <DialogContent className="w-[480px] max-w-[calc(100vw-32px)] p-0 flex flex-col">
        <div className="px-6 pt-6 pb-5 border-b border-ink-150">
          <DialogTitle className="text-[16px]">
            删除项目 <span className="text-ink-700">「{target?.name}」</span>
          </DialogTitle>
          <div className="text-[13px] text-ink-500 mt-1">
            项目下的所有方案与版本将一并归档，可由项目所有者从归档恢复
          </div>
        </div>
